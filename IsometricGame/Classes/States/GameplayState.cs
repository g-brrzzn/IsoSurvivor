...
using IsometricGame.Classes.Items;using IsometricGame.Classes.Particles;
using IsometricGame.Classes.UI;using IsometricGame.Classes.Upgrades;
using IsometricGame.Classes.Weapons.Projectiles;using IsometricGame.Classes.Physics;
...
namespace IsometricGame.States
{
    ...
    public class GameplayState : GameStateBase
    {
        ...
        {
            base.Start();
            _spatialGrid = new SpatialGrid(100);
        ...
        private void HandleCollisions(GameTime gameTime)
        {
            ...
            var enemiesNearPlayer = _spatialGrid.Retrieve(new Vector2(GameEngine.Player.WorldPosition.X, GameEngine.Player.WorldPosition.Y));
            ...
            foreach (var enemy in enemiesNearPlayer)
            {
                ...
                if (Vector2.DistanceSquared(new Vector2(enemy.WorldPosition.X, enemy.WorldPosition.Y),
                                            new Vector2(GameEngine.Player.WorldPosition.X, GameEngine.Player.WorldPosition.Y)) < 0.64f)
                {
            ...
            for (int j = GameEngine.PlayerBullets.Count - 1; j >= 0; j--)
            {
                ...
                var nearbyEnemies = _spatialGrid.Retrieve(new Vector2(bullet.WorldPosition.X, bullet.WorldPosition.Y));

                foreach (var enemy in nearbyEnemies)
                {
                    if (bullet.IsRemoved) break;                    if (enemy.IsRemoved || bullet.HitList.Contains(enemy)) continue;
            ...
            foreach (var sprite in GameEngine.AllSprites)
            {
                ...
                {
                    var nearbyEnemies = _spatialGrid.Retrieve(new Vector2(orb.WorldPosition.X, orb.WorldPosition.Y));
                    ...
                    foreach (var enemy in nearbyEnemies)
                    {
                        ...
                        if (Vector2.DistanceSquared(new Vector2(orb.WorldPosition.X, orb.WorldPosition.Y),
                                                   new Vector2(enemy.WorldPosition.X, enemy.WorldPosition.Y)) < orbRadiusSq)
                        {
        ...
        {
            int margin = 200;            Vector2 camPos = Game1.Camera.Position;
        ...
        private void DrawHUD(SpriteBatch spriteBatch)
        {
            ...
            if (_waveManager != null)
            {
                ...
                DrawUtils.DrawTextScreen(spriteBatch, timeStr, font, new Vector2(screenW / 2, barY + 35), Color.Gold, 0f);
            }
        }
